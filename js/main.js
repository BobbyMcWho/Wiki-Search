  ...
  function wikiSearch() {
    $.ajax({
      ...
      complete: function() {
        ...
        if (queryResults[1] == ""){$('#father').append('<a class="card" target="_blank">' + '<div class="son"><h2 class="cardHead">No results found.</h2>' + '<br>' + '<p>Try another search query!</p></div></a>');;}
        else loopResults(queryResults);
      }
    });
  }
